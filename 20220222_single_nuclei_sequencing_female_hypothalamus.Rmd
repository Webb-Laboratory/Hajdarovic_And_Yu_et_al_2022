...
dir = "~/Desktop/single_cell_sequencing_paper/"
setwd(dir)
...
met<-read_csv("~/Downloads/GSE174367_snRNA-seq_cell_meta.csv")
table(met$SampleID, met$Sex)
...
AD_3 <- Read10X(data.dir = "AD3_filtered_feature_bc_matrix") 
CTRL_3 <- Read10X(data.dir = "CTRL3_filtered_feature_bc_matrix") 
...
library(ggrepel)
#load hypothalamus DE data
cell_DE_res_haj<-readRDS("DE_res_all_cells_sample_effect_20210817.RDS")
...
bulk_MAST<-readRDS("bulk.DE.RES.20210721.RDS")
micro<-read_csv("microarray.csv")


micro<-mutate(micro, updown = ifelse(FC > 0, "up", "down"))


#bulk_MAST<-mutate(bulk_MAST, sig = ifelse( p_val_adj < 0.05 & avg_log2FC > 0.1, "up", ifelse (p_val_adj < 0.05 & avg_log2FC < -0.1, "down", "NS")))
                                           
bulk_MAST<-mutate(bulk_MAST, sig = ifelse(p_val_adj < 0.05 & avg_log2FC > 0.0, "up", ifelse (p_val_adj < 0.05 & avg_log2FC < 0.0, "down", "NS")))


bulk_MAST<-dplyr::filter(bulk_MAST, gene %in% micro$Symbol)

x<-full_join(micro, bulk_MAST, by = "gene")
